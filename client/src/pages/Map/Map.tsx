...
const ResetCenterView: React.FC<{ position: [number, number] | null }> = ({
  position,
}) => {
  const map = useMap();
...
const OpenStreetMap: React.FC = ({}) => {
  ...
  const [showRoutePointsSearchResult, setShowRoutePointsSearchResult] =
    useState<boolean>(false);
  const [routePointsSearchText, setRoutePointsSearchText] =
    useState<string>("");
  const [routePointSearchResults, setRoutePointSearchResults] = useState<
    Array<any>
  >([]);
              ...
              <div className="flex flex-col items-center justify-center">
                <div className="search-results items-center">
                  {places.map((item) => {
                    return (
                      <div
                        className="flex items-center m-2 p-1 border-solid border-2 shadow-md shadow-slate-200 rounded-md"
                        key={item?.place_id}
                        onClick={() =>
                          setSearchPosition([item?.lat, item?.lon])
                        }
                      >
                        <div>
                          <Icon
                            as={MapPin}
                            label="map pin"
                            className="w-6 h-6 mx-3 text-grey-500"
                          />
                        </div>
          ...
          <div className="container flex flex-col">
            ...
            <div className="search-box flex-col md:flex-row">
              ...
              <Button
                variant="solid"
                onClick={async () => {
                  await handleSearchRoutePointPlace();
                }}
                leftIcon={
                  <Icon as={Search} label="search" className="w-4 h-4 mr-1" />
                }
